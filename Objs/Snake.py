...
class Snake(Obj):
    ...
    def eat(self, screen_size):#Добавлено для единообразия
        if not self.tail:
            cop=self.phys.copy()
            cop.geom.center-=self.vel
            self.tail.append(Obj(cop, self.vel))##Интересная проблема с указателями в питоне, нужно будет рассказать
        cop=self.tail[-1].phys.copy()
        cop.geom.center-=self.tail[-1].vel
        self.tail.append(Obj(cop, self.tail[-1].vel))
